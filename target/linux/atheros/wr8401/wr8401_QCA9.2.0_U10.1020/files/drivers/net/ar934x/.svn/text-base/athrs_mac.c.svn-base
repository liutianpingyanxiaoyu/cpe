...
//#define LINK_INTR_ENABLE   //LINK INTR have issue: can't clear interrupt bit.
...
athr_gmac_open(struct net_device *dev)
{
    ...
    */
    ...
    athr_gmac_reg_wr(mac, 0x2c,0xffff);
    ...
    */
    ...
    if (mac_has_flag(mac,ETH_LINK_INTERRUPT)) {
        if (athr_gmacs[0]->mac_ifup == 0) {
            if (is_ar934x() && is_s17()) {
                st = request_irq(ATHR_GPIO_IRQn(4), athr_gmac_link_intr, 
                         ATHR_MAC_IRQF_DISABLED, "gmac s17 link intr", athr_gmacs[0]);
                if (st < 0) {
                    printk(MODULE_NAME ": request irq %d failed %d\n"
                           , ATHR_IRQ_ENET_LINK, st);
                    free_irq(ATHR_IRQ_ENET_LINK, mac->mac_dev); 
                ...
                {
                    /* enable link interrupt on GPIO4*/
                   
                    ath_gpio_config_input(4);
                    
                    /* configure GPIO4 as level triggered interrupt */
                    ath_gpio_config_int(4, INT_TYPE_LEVEL, INT_POL_ACTIVE_LOW);
                }
            }
        }
    } 
