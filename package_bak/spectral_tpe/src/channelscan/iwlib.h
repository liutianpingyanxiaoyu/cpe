...
/* My definition Added by Bruce Liu, 2013-9-9 */
struct quality_pair
{
...
typedef struct quality_pair *quality_pair_t;

struct scan_database
{
	struct scan_database *cell_next;
	struct scan_database *cell_pre;
	int cell_number;
	char cell_mac_addr[24];
	int cell_frequency;
	struct quality_pair cell_quality_pair;
	int cell_signal_level;
	int cell_noise_floor_level;
};

typedef struct scan_database *scan_database_t;
...
struct cell_point
{
	...
	int count;   /* this channel scan count of ap */
};
...
struct scan_summary
{
	int channel[254]; /* channel number: 1~254 */
	struct cell_point cell[254];     /* corresponding to channel number: 1~255 */
	int max_rssi[254];   /* max rssi of corresponding to channel number */

	int noise_floor[254]; /* corresponding to channel number: 1~255 */
};

typedef struct scan_summary *scan_summary_t;
/* Ended by Bruce Liu */
