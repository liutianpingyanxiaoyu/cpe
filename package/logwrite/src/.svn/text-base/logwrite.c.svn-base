...
void filt_f_message(char *msg, char *keycat)
{
    ...
    /* level and category */
    strncpy(cur, msg , p1 - p2 + 1); 
    cur = cur + (p1 - p2 + 1) ;
    snprintf(cur, strlen(keycat) + 5, "\t<%s>\t", keycat);
    cur = cur + (strlen(keycat) + 4);

    /* time */
    p1 += 1;
    n = snprintf(cur, p3 - p1, "%s", p1);
    cur = cur + (p3 - p1) - 1;
...
void filt_U_message(char *msg, char *keycat)
{
    ...
    /* <13>Jul 27 11:27:46 (none) root: EZP_USR ACL Database rt: start [OK] */
    p1 = strchr(cur, '>');
    p3 = strstr(cur, "New USB device found,");
    ...
    snprintf(cur - 1, msg + MSGBUF_SIZE - cur, " Find USB device: %s", p3);
}
...
int logwrite_main(int argc, char **argv)
{
    ...
    while(1) {
        int c = getopt(argc, argv, "k:l:c:s:fUuriptPdDhKb") ;
        if(c == -1)
            break ;
