...
#define	USAGE_PRINTBD  \
 	"\nprint_bd [[ -n name ] | [ name ... ]]\n" \
 ...
 */
int bd_printenv (int argc, char *argv[])
{
	char *env, *nxt;
	int i, n_flag;
	int rc = 0;
...
int isValidMacAddress(const char* mac) {
	...
	while (*mac) {
		if (isxdigit(*mac)) { //Checks for a hex digit
			i++;
		} else if (*mac == ':' /*|| *mac == '-'*/) { //Checks for delimitation 
			if (i == 0 || (i / 2 - 1) != s)
				break;
			++s;
		} else {
 ...
 *
 */
int bd_setenv (int argc, char *argv[])
{
	int i, len;
	char *env, *nxt;
	char *oldval = NULL;
	 ...
	 *Checks for mac address 
	 */
	if ((strcmp (name, "mac") == 0) || (strcmp (name, "MAC") == 0)) {
		if (argc != 3 || !isValidMacAddress(argv[2])) {
			fprintf (stderr, "Error: mac address invalid. i.e set_bd mac xx:xx:xx:xx:xx:xx.\n");
			errno = EINVAL;
			return -1;
		}
	}
  ...
  WRITE_FLASH:
	...
	*environment.crc = uboot_crc32 (0, (uint8_t *) environment.data, ENV_SIZE);
	...
	if (flash_io (O_RDWR)) {
		fprintf (stderr, "Error: can't write board data to flash\n");
		return -1;
	}
...
static int env_init (void)
{
...
#ifdef DBUG
	fprintf (stderr,
			"DBUG: environment CONFIG_ENV_SIZE: %d \n",
			CONFIG_ENV_SIZE);

	fprintf (stderr,
			"DBUG: environment ENV_SIZE: %d \n",
			ENV_SIZE);
#endif
	...
	if (!HaveRedundEnv) {
		if (!crc0_ok) {
			fprintf (stderr,
				"Warning: Bad CRC, if you have not set board data before, Set it first.\n");
			fprintf (stderr,
				"if you are setting board data, ignore this warning.\n");
#if 0
			fprintf (stderr,
	...
	} else { //HaveRedundEnv. we don't have redundant env.
