...
void spectral_scan_classifer_sm_init(CLASSIFER_DATA_STRUCT *pclas, DETECT_MODE mode)
{
    ...
    if (mode & SPECT_CLASS_DETECT_MWO) {
        ...
        pclas->mwo_cur_burst_cnt    = 0;
        pclas->mwo_pre_burst_time   = 0;
        pclas->mwo_thresh           = 3000;
        pclas->mwo_rssi             = 0;
        pclas->mwo_cur_bin          = 0;
...
{
    int ret_val = 0;
    spectral_mwo_param *cur_mwo = &pclas->mwo_param[pclas->mwo_cur_bin];

    /* Check if this is a valid microwave oven frequency */
    if (msg->freq < GET_MIN_MWO_FREQ(pclas)) return 0;
    if (msg->freq > GET_MAX_MWO_FREQ(pclas)) return 0;


    /* Check if there is a narrow band peak */
    if (msg->samp_data.spectral_rssi > GET_MIN_RSS_TO_DETECT(pclas)) {
        ...
        if (peak_bin + (GET_MWO_INT_BIN_SUM_SIZE(pclas) + (GET_MWO_INT_BIN_SUM_SIZE(pclas) >> 1)) <= msg->samp_data.bin_pwr_count) {
            chk_upr = 1;
        }

        if ((int)peak_bin - (GET_MWO_INT_BIN_SUM_SIZE(pclas) + (GET_MWO_INT_BIN_SUM_SIZE(pclas) >> 1)) <= 0) {
            chk_lwr = 1;
        }

        /* set the upper and lower bin markers */
        /* XXX : Note, this logic works only for GET_MWO_INT_BIN_SUM_SIZE(pclas) = 3 */
        ...
        /* XXX : Note, this logic works only for GET_MWO_INT_BIN_SUM_SIZE(pclas) = 3 */
        /* Check if this is greater then threshold */

        if ( (lwr_sum < (center_sum >> 2)) &&
             (upr_sum < (center_sum >> 2)) &&
             (center_sum > GET_MWO_INT_DET_THRESH(pclas))) {

              /* Looks like a MWO burst. Log it */
              spectral_scan_log_data(msg, pclas, SPECT_CLASS_DETECT_MWO, 0);

              if (!cur_mwo->in_use) {
                  /* This is being used for the first time */
                  cur_mwo->in_use   = 1;
                  cur_mwo->start_ts = msg->samp_data.spectral_tstamp;
                  cur_mwo->last_ts  =  msg->samp_data.spectral_tstamp;
                  cur_mwo->rssi     += msg->samp_data.spectral_rssi;
                  cur_mwo->num_samp++;
              } else {

                  /* This has been in use, check how long this burst has been there */
                  if ((int)(msg->samp_data.spectral_tstamp - cur_mwo->last_ts) < GET_MWO_MAX_GAP_WITHIN_BURST(pclas)) {

                      /* This is the current burst */
                      cur_mwo->last_ts  =  msg->samp_data.spectral_tstamp;
                      cur_mwo->rssi     += msg->samp_data.spectral_rssi;
                      cur_mwo->num_samp++;

                      /* Check how long it has been on */
                      if ((int)(cur_mwo->last_ts - cur_mwo->start_ts) > GET_MWO_MAX_BURST_TIME(pclas)) {
                          /* This burst is too long, must be CW interference. Reset detection */
                          spectral_scan_classifer_sm_init(pclas, SPECT_CLASS_DETECT_MWO);
                      }

                  } else {

                      /* This is a new burst. Save the delta between start and stop of burst */
                      cur_mwo->delta = (int)(cur_mwo->last_ts - cur_mwo->start_ts);
                      pclas->mwo_cur_bin++;

                      if (pclas->mwo_cur_bin == NUM_MWO_BINS) {

                          /* Figure out if there is an MWO interference */
                          u_int32_t tot_rssi    = 0;
                          u_int32_t num_detect  = 0;
                          u_int32_t on_time     = 0;
                          u_int32_t off_time    = 0;

                          /* Compute the average on and off time */
                          for (bin_cnt = 0; bin_cnt < NUM_MWO_BINS-1; bin_cnt++) {

                              tot_rssi      += pclas->mwo_param[bin_cnt].rssi;
                              num_detect    += pclas->mwo_param[bin_cnt].num_samp;
                              on_time       += pclas->mwo_param[bin_cnt].delta;
                              pclas->mwo_param[bin_cnt].off_time =
                                      (int)(pclas->mwo_param[bin_cnt+1].start_ts -
                                      pclas->mwo_param[bin_cnt].last_ts);
                              off_time += pclas->mwo_param[bin_cnt].off_time;
                          }
                          ...
                          on_time /= NUM_MWO_BINS;
                          off_time /= (NUM_MWO_BINS-1);
                          tot_rssi /= num_detect;

                          //printf("on_time %d off_time %d average_rssi %d num_detect %d\n", on_time, off_time, tot_rssi, num_detect);
                          /* Check if the average on and off times are within range */
                          if ( (on_time > GET_MWO_MIN_DUTY_CYCLE(pclas)) &&
                               (on_time < GET_MWO_MAX_DUTY_CYCLE(pclas)) &&
                               (off_time > GET_MWO_MIN_DUTY_CYCLE(pclas)) &&
                               (off_time < GET_MWO_MAX_DUTY_CYCLE(pclas))) {

                                /* check if more the half of the bursts are within range */
                                u_int16_t within_range = 0;

                                for (bin_cnt = 0; bin_cnt < (NUM_MWO_BINS-1); bin_cnt ++ ) {
                                    if ((pclas->mwo_param[bin_cnt].off_time > GET_MWO_MIN_DUTY_CYCLE(pclas)) &&
                                         (pclas->mwo_param[bin_cnt].off_time < GET_MWO_MAX_DUTY_CYCLE(pclas)) &&
                                         (pclas->mwo_param[bin_cnt].delta > GET_MWO_MIN_DUTY_CYCLE(pclas)) &&
                                         (pclas->mwo_param[bin_cnt].delta < GET_MWO_MAX_DUTY_CYCLE(pclas))) {
                                               within_range++;
                                    }

                                    //printf("On time %d off time %d\n", pclas->mwo_param[bin_cnt].delta, pclas->mwo_param[bin_cnt].off_time);
                                }

                                //printf("Within range %d\n", within_range);
                                /* Check if at least half the bursts are within range */
                                if (within_range >= (NUM_MWO_BINS >> 1)) {
                                    /* This looks like a MWO burst */
                                    ret_val = 1;
                                }
                          }
                          ...
                          /* Log a new burst */
                          cur_mwo = &pclas->mwo_param[pclas->mwo_cur_bin];

                          /* Put the burst in a new bin */
                          cur_mwo->in_use   = 1;
                          cur_mwo->start_ts = msg->samp_data.spectral_tstamp;
                          cur_mwo->last_ts  =  msg->samp_data.spectral_tstamp;
                          cur_mwo->rssi     += msg->samp_data.spectral_rssi;
            ...
            /* Not a narrow band burst. This is a current burst, check if this has been on for too long */
            if ((int)(msg->samp_data.spectral_tstamp - cur_mwo->last_ts) > GET_MWO_MAX_BURST_TIME(pclas)) {
    ...
    /* We found a MWO interference once. Check if it is persistant within a given time */
    if (ret_val) {
        ...
        if (!(pclas->current_interference & SPECT_CLASS_DETECT_MWO)) {
            if (!pclas->mwo_num_detect) {
                ...
                /* Check MWO has been detected enough number of times */
                if (((int)(msg->samp_data.spectral_tstamp - pclas->mwo_detect_ts) < GET_MWO_SECOND_DET_THRESH(pclas)) &&
                      (pclas->mwo_num_detect >= GET_MWO_DETECT_CONFIRM_COUNT(pclas))) {
                    pclas->current_interference |= SPECT_CLASS_DETECT_MWO;
                    pclas->mwo_detect_ts = msg->samp_data.spectral_tstamp;
                    printf("Found MWO interference in frequency %d\n", msg->freq);
                    ret_val = 1;
                    pclas->mwo_cnt++;
        ...
        if ((int)(msg->samp_data.spectral_tstamp - pclas->mwo_detect_ts) > GET_MWO_CONFIRM_MISSING_TIME(pclas)) {
            pclas->current_interference &=  ~(SPECT_CLASS_DETECT_MWO);
            pclas->mwo_num_detect = 0;
            printf("No MWO interference\n");
        }
    }
    return ret_val;
}
    ...
    if (msg->samp_data.spectral_rssi > GET_WIFI_DET_MIN_RSSI(pclas)) {
        ...
        if (ch_width == IEEE80211_CWM_WIDTH20) {
            ...
            if (!found) {
                ...
                found = FALSE;
                printf("Invalid Channel bandwidth\n");
        }
            ...
            if (!pclas->wifi_num_detect) {
                ...
                if ((int)(msg->samp_data.spectral_tstamp - pclas->wifi_detect_ts) < GET_WIFI_DET_CONFIRM_WIN(pclas)) {
                    ...
                    if (pclas->wifi_num_detect >= WIFI_MIN_NUM_DETECTS) {
                        ...
                        printf("Found WiFi interference in freq %d with RSSI %d\n", msg->freq, pclas->wifi_rssi);
        ...
        if (!(pclas->current_interference & SPECT_CLASS_DETECT_FHSS)) {
            ...
            if (!pclas->fhss_num_detect) {
                ...
                if ((int)(msg->samp_data.spectral_tstamp - pclas->fhss_detect_ts) < GET_FHSS_DETECTION_CONFIRM_WIN(pclas)) {
                    ...
                    printf("Found FHSS interference\n");
                    pclas->fhss_cnt++;
