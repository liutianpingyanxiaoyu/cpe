...
define KernelPackage/ipt-trigger
  SUBMENU:=$(NF_MENU)
  TITLE:=Module for user-space packet triggering
  KCONFIG:=$(KCONFIG_IPT_TRIGGER)
  FILES:=$(foreach mod,$(IPT_TRIGGER-m),$(LINUX_DIR)/net/$(mod).$(LINUX_KMOD_SUFFIX))
  AUTOLOAD:=$(call AutoLoad,45,$(notdir $(IPT_TRIGGER-m)))
  DEPENDS:= kmod-ipt-core
endef

define KernelPackage/ipt-trigger/description
 Netfilter (IPv4) module for user-space packet triggering
 Includes:
 - ipt_TRIGGER
endef
