...
ifeq ($(DUMP),1)
  ...
  ifeq ($(TARGET_BUILD),1)
    #Modify for subtarget files, Sean@liteon, 2013-03-28----->
    #PATCH_DIR ?= ./patches$(if $(wildcard ./patches-$(KERNEL_PATCHVER)),-$(KERNEL_PATCHVER))
    #FILES_DIR ?= $(foreach dir,$(wildcard ./files ./files-$(KERNEL_PATCHVER)),"$(dir)")
    #
    #Get right target patches from top target dir or sub target dir.
    #
    ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
      TOP_PATCH_DIR?=./patches$(shell [ -d "./patches-$(KERNEL_PATCHVER)" ] && printf -- "-$(KERNEL_PATCHVER)" || true )
      SUB_PATCH_DIR?=./$(SUBTARGET)/patches$(shell [ -d "./$(SUBTARGET)/patches-$(KERNEL_PATCHVER)" ] && printf -- "-$(KERNEL_PATCHVER)" || true )
      PATCH_DIR?=$(shell [ -d "$(SUB_PATCH_DIR)" ] && printf -- "$(SUB_PATCH_DIR)" || printf -- "$(TOP_PATCH_DIR)" )
     else
      PATCH_DIR ?= ./patches$(shell [ -d "./patches-$(KERNEL_PATCHVER)" ] && printf -- "-$(KERNEL_PATCHVER)" || true )
    endif

    #
    #Get right target files from top target dir or sub target dir.
    #
    ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
     TOP_FILES_DIR ?= ./files$(shell [ -d "./files-$(KERNEL_PATCHVER)" ] && printf -- "-$(KERNEL_PATCHVER)" || true )
     SUB_FILES_DIR ?=  ./$(SUBTARGET)/files$(shell [ -d "./$(SUBTARGET)/files-$(KERNEL_PATCHVER)" ] && printf -- "-$(KERNEL_PATCHVER)" || true )
     FILES_DIR ?=$(shell [ -d "$(SUB_FILES_DIR)" ] && printf -- "$(SUB_FILES_DIR)" || printf -- "$(TOP_FILES_DIR)" )
   else
     FILES_DIR ?=  ./files$(shell [ -d "./files-$(KERNEL_PATCHVER)" ] && printf -- "-$(KERNEL_PATCHVER)" || true )
   endif
   #<-----Sean@liteon, 2013-03-28
  endif
