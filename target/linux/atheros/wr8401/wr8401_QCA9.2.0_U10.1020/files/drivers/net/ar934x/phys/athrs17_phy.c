...
static uint8_t athr17_setled_flag = 0;  /* flag to record led configured*/
...
*/
#define FLUSH_ARL_CACHE 0x80000001
#define CLEAR_FUNC_REG 0x0
...
athrs17_phy_is_up(int ethUnit)
{
    ...
    /* workaround: S17_LED_CTRL3_REG will be changed at setup, so set led behavior again here*/
    if (athr17_setled_flag == 0)
    {
        /* set LED behavior*/
        athrs17_reg_write(S17_PWS_REG, athrs17_reg_read(S17_PWS_REG) | (1 << 31));
    ...
    for (phyUnit=0; phyUnit < S17_PHY_MAX; phyUnit++) {
        ...
        if (lastStatus->isPhyAlive) { /* last known link status was ALIVE */
            ...
            if (phyHwStatus & S17_STATUS_LINK_PASS) {
                ...
                /* Reset port PHY: workaround for 1000M negotiate to 100M. */
                phy_reg_write(phyBase, phyAddr, S17_PHY_CONTROL,
                              S17_CTRL_AUTONEGOTIATION_ENABLE 
                              | S17_CTRL_SOFTWARE_RESET);
                ...
                /*added by Bruce Liu. 2012-03-28*/
                athrs17_reg_write(S17_ATU_FUNC_REG, FLUSH_ARL_CACHE);
                athrs17_reg_write(S17_ATU_FUNC_REG, CLEAR_FUNC_REG);
                /*End by Bruce*/
