...
static int get_http_info(const struct sk_buff *skb, int flags, httpinfo_t *info)
{
    ...
    if (memcmp(data, "GET ", sizeof("GET ") - 1) != 0 &&
        memcmp(data, "POST ", sizeof("POST ") - 1) != 0 &&
        memcmp(data, "HEAD ", sizeof("HEAD ") - 1) != 0) /* EZP:Added for filtering HTTP HEAD */
	return ret;	/* Pass it */
    ...
    *(info->host + hostlen) = 0;		/* null-terminated */
    ...
    /* EZP:Added for filtering HTTP HEAD */
    if (!found)
...
)
{
	...
	switch(info->type)
	{
	    ...
	    default:
		SPARQ_LOG("%s: Sorry! Cannot find this match option.\n", __FILE__);
		return false;
	}
	...
	if (info->type == IPT_WEBSTR_HOST || info->type == IPT_WEBSTR_URL) {
        ...
        /* Treat flash .swf file as some kind of activeX */
		vicelen = sizeof(".swf") - 1;
		if (strnicmp(htinfo.url + htinfo.urllen - vicelen, ".swf", vicelen) == 0) {
		    SPARQ_LOG("%s: MATCH....activex\n", __FUNCTION__);
		    found = 1;
		    goto match_ret;
		}
