...
IMPLEMENT_DYNAMIC(CNGWizard, CNGWizard_BASE)
...
BEGIN_MESSAGE_MAP(CNGWizard, CNGWizard_BASE)
...
{
	BOOL bResult = CNGWizard_BASE::OnInitDialog();
	...
	if (1)//Is256ColorSupported())
	{
//		SetWatermark(IDB_WATERMARK256,	WIZ97_BITMAP_STRETCH);
//		SetHeader(IDB_BITMAP1,		WIZ97_BITMAP_STRETCH);
	}
	else
	{
//		SetWatermark(IDB_WATERMARK16,	WIZ97_BITMAP_STRETCH);
//		SetHeader(IDB_BANNER16,			WIZ97_BITMAP_STRETCH);
	}
...
{
	BOOL bResult = CNGWizard_BASE::OnEraseBkgnd(pDC);
...
BOOL CNGWizard::OnQueryNewPalette(void) 
{
	...
	return CNGWizard_BASE::OnQueryNewPalette();
}
