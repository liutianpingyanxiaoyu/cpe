...
//void import(char *filename, int offset, int len)
{
    ...
    FILE *fprod; //aron add
    char cmd[1024];
 	//Gen tmp config file '/var/uploadCfg' from stream
    ...
    if( fseek(src, offset+33, SEEK_SET) == -1){
        printf("fseek error\n");
    }
    while( len - 33 > 0){
	...
	//Put firstline from stream to '/var/uploadCfg.md5'
    if(!( fp = fopen("/var/uploadCfg.md5", "w+"))){
      return 1;
    }
    if(!( src = fopen(filename, "r"))){
      fclose(fp);
	...
	char upload_md5[64] = {0};
	char config_md5[64] = {0};

	//Gen md5 from tmp config '/var/uploadCfg' to 'var/config.md5'
	system("md5sum /var/uploadCfg | cut -d' ' -f 1 > /var/config.md5");

	//fscanf from '/var/uploadCfg.md5' (first line in upload file)
    if(( fp = fopen("/var/uploadCfg.md5", "r"))==NULL){
      	return 1;
    }
	else {
		fscanf(fp, "%s\n", upload_md5);
		close(fp);	
		system("rm -rf /var/uploadCfg.md5");
	}

	//fscanf from '/var/config.md5' (md5 value gen from md5sum)
    if(( fp = fopen("/var/config.md5", "r"))==NULL){
      	return 1;
    }
	else {
		fscanf(fp, "%s\n", config_md5);
		close(fp);	
		system("rm -rf /var/config.md5");
	}

	//compare 
	if( strncmp(config_md5, upload_md5, 32) ) {
		return 1;
	}
	
	//origin procedure without md5sum check
	...
	ret = system("nvram import /var/uploadCfg");
    if(ret) {
		return 1;
	}
...
<body> ");
	...
	system("echo 1 > /tmp/uploadConfigOK");
    ...
    else
    {
      ...
      printf("</body> <script>function waitingpage(){top.location.href = '/local/advance/proceeding2.asp';}waitingpage();</script></html>");
    }
err:
