...
int nvram_init_ralink(int index, int * nvram_fd) 
{
...
#endif
	int ret;
	//printf("+++++++++++ %s ++++++++++++  part 0 ,index is %d\n",__func__,index);
	LIBNV_PRINT("--> nvram_init %d\n", index);
	LIBNV_CHECK_INDEX();
	//printf("+++++++++++ %s ++++++++++++  part 0-1 ,valid is %d\n",__func__,fb[index].valid);
	//if (CONFIG_KERNEL_NVRAM)
	//printf("+++++++++++ %s ++++++++++++  part 0-2 ,CONFIG_KERNEL_NVRAM  is valid\n",__func__);
	if (fb[index].valid)
		return 0;
	...
	*nvram_fd = fd = open(NV_DEV, O_RDONLY);//here maybe not useful because write is use ioctl in the below set also open the NV_DEV with 	O_RDONLY ,commented by daniel
	...
	if (fd < 0) {
		perror(NV_DEV);
		ret=fd;
		//printf("+++++++++++ %s ++++++++++++  part 3 ++++ *nvram_fd is %d ,fd is %d\n",__func__,*nvram_fd,fd);
		goto out;
	}
	//printf("+++++++++++ %s ++++++++++++  part 4\n",__func__);
	if (ioctl(fd, RALINK_NVRAM_IOCTL_GETALL, &nvr) < 0) {
...
char *nvram_get_ralink(int index, char *name)
{
	//LIBNV_PRINT("--> nvram_get\n");
...
int nvram_getall_ralink(int index, char *buf,int count)
{
	...
	//fd = open(NV_DEV, O_RDONLY);
	//printf("+++++++++++ %s ++++++++++++  part 1\n",__func__);
	 fd = open(NV_DEV, O_RDONLY);//here maybe not useful because write is use ioctl in the below set also open the NV_DEV with 	O_RDONLY ,commented by daniel
	//printf("=========*nvram_fd %d= fd=%d===========\n", *nvram_fd,fd);
	//printf("+++++++++++ %s ++++++++++++  part 2\n",__func__);
	if (fd < 0) {
		perror(NV_DEV);
