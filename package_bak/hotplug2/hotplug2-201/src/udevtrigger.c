...
static void trigger_uevent(const char *devpath)
{
	...
	strncpy(filename, "/sys", sizeof(filename));
	strncat(filename, devpath, sizeof(filename));
...
static int sysfs_resolve_link(char *devpath, size_t size)
{
	...
	strncpy(link_path, "/sys", sizeof(link_path));
	strncat(link_path, devpath, sizeof(link_path));
	...
	strncat(devpath, "/", size);
	strncat(devpath, &link_target[back * 3], size);
	return 0;
}
...
static int device_list_insert(const char *path)
{
	...
	strncpy(devpath, &path[4], sizeof(devpath));
...
static void scan_subsystem(const char *subsys)
{
	...
	strncpy(base, "/sys/", sizeof(base));
	strncat(base, subsys, sizeof(base));
	...
	if (dir != NULL) {
		for (dent = readdir(dir); dent != NULL; dent = readdir(dir)) {
			...
			strncpy(dirname, base, sizeof(dirname));
			strncat(dirname, "/", sizeof(dirname));
			strncat(dirname, dent->d_name, sizeof(dirname));
			strncat(dirname, "/devices", sizeof(dirname));
			...
			if (dir2 != NULL) {
				for (dent2 = readdir(dir2); dent2 != NULL; dent2 = readdir(dir2)) {
					...
					strncpy(dirname2, dirname, sizeof(dirname2));
					strncat(dirname2, "/", sizeof(dirname2));
					strncat(dirname2, dent2->d_name, sizeof(dirname2));
...
static void scan_block(void)
{
	...
	strncpy(base, "/sys/block", sizeof(base));
...
static void scan_class(void)
{
	...
	strncpy(base, "/sys/class", sizeof(base));
...
int main(int argc, char *argv[], char *envp[])
{
	...
	strncpy(base, "/sys/class/block", sizeof(base));
