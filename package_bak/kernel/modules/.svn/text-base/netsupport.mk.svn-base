...
# Compatible with the different Kernel version, <Sean@Liteon.com>, 2014.03.06
ifeq ($(call kernel_ifversion, -ge, 030200),y)
  PPP_DIR:=$(LINUX_DIR)/drivers/net/ppp
  SLIP_DIR:=$(LINUX_DIR)/drivers/net/slip
else
  PPP_DIR:=$(LINUX_DIR)/drivers/net
  SLIP_DIR:=$(LINUX_DIR)/drivers/net
endif
...
define KernelPackage/ppp
  ...
  KCONFIG:= \
	...
	CONFIG_SLHC
  FILES:= \
	$(PPP_DIR)/ppp_async.ko \
	$(PPP_DIR)/ppp_generic.ko \
	$(SLIP_DIR)/slhc.ko 
...
# Compatible with the different Kernel version, <Sean@Liteon.com>, 2014.03.06
ifeq ($(call kernel_ifversion, -le, 020631),y)
  L2TP_FILE_NAME:=pppol2tp
  L2TP_FILES:=$(LINUX_DIR)/drivers/net/$(L2TP_FILE_NAME).ko
else
  L2TP_FILE_NAME:=l2tp_ppp
  L2TP_FILES:=$(LINUX_DIR)/net/l2tp/$(L2TP_FILE_NAME).ko
endif
...
define KernelPackage/pppol2tp
  ...
  FILES:=$(L2TP_FILES)
  AUTOLOAD:=$(call AutoLoad,41,$(L2TP_FILE_NAME))
endef
