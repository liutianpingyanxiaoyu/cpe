...
void
iw_calcu_freq_value(char *	buffer,
		    int		buflen,
		    double	freq)
{
  if(freq < KILO)
	      ...
	      int	freq_flags)
{
  ...
  /* Print the frequency/channel value */
  iw_calcu_freq_value(vbuf, sizeof(vbuf), freq);
...
void
iw_process_stat(char *		buffer,
	       int		buflen,
	       const iwqual *	qual,
	       const iwrange *	range,
	       int		has_range,
	       struct scan_database *result)
{
  int		len;
  ...
  if(has_range && ((qual->level != 0)
		   || (qual->updated & (IW_QUAL_DBM | IW_QUAL_RCPI))))
    {
      ...
      if(!(qual->updated & IW_QUAL_QUAL_INVALID))
	{
	  result->cell_quality_pair.denominator =  qual->qual;
	  result->cell_quality_pair.numerator = range->max_qual.qual;
	  buffer += len;
	  buflen -= len;
	}
      ...
      if(qual->updated & IW_QUAL_RCPI)
	{
	  ...
	  if(!(qual->updated & IW_QUAL_LEVEL_INVALID))
	    {
	      double	rcpilevel = (qual->level / 2.0) - 110.0;
	      result->cell_signal_level = rcpilevel;
	      buffer += len;
	      buflen -= len;
	    }
	  ...
	  if(!(qual->updated & IW_QUAL_NOISE_INVALID))
	    {
	      double	rcpinoise = (qual->noise / 2.0) - 110.0;
	     result->cell_noise_floor_level = rcpinoise;
	    }
	}
      else
	{
	  ...
	  if((qual->updated & IW_QUAL_DBM)
	      ...
	      if(!(qual->updated & IW_QUAL_LEVEL_INVALID))
		{
		  ...
		  if(qual->level >= 64)
		    dblevel -= 0x100;
		  result->cell_signal_level = dblevel;
		  buffer += len;
		  buflen -= len;
		}
	      ...
	      if(!(qual->updated & IW_QUAL_NOISE_INVALID))
		{
		  ...
		  if(qual->noise >= 64)
		    dbnoise -= 0x100;
		  result->cell_noise_floor_level = dbnoise;
		}
	    }
	  else
	    {
	      ...
	      if(!(qual->updated & IW_QUAL_LEVEL_INVALID))
		{
		  /*len = snprintf(buffer, buflen, "Signal level%c%d/%d  ",
				 qual->updated & IW_QUAL_LEVEL_UPDATED ? '=' : ':',
				 qual->level, range->max_qual.level);*/
	           result->cell_signal_level = qual->level;
		  buffer += len;
		  buflen -= len;
		}
	      ...
	      if(!(qual->updated & IW_QUAL_NOISE_INVALID))
		{
		  /*len = snprintf(buffer, buflen, "Noise level%c%d/%d",
	      	  ...
	      	  result->cell_noise_floor_level =  qual->noise;
		}
	    }
	}
    }
