...
#Modify image name, Sean@liteon, 2013-01-01
#IMG_PREFIX:=openwrt-$(BOARD)$(if $(SUBTARGET),-$(SUBTARGET))
IMG_PREFIX:=$(if $(BOARDNAME),$(BOARDNAME))
#<-----Sean@liteon,2013-01-01
...
#Code for make ubifs image, Sean@liteon, 2013-01-01----->
ifneq ($(CONFIG_TARGET_ROOTFS_UBIFS),)
  UBIFS_OPTS := $(call qstrip,$(CONFIG_UBIFS_OPTS)) 
  UBINIZE_OPTS := $(call qstrip,$(CONFIG_UBINIZE_OPTS))
  UBIFS_IMAGE_NAME := $(call qstrip,$(CONFIG_UBIFS_IMAGE_NAME))
  VOLUME_ID  := $(CONFIG_VOLUME_ID)
  VOLUME_SIZE := $(CONFIG_VOLUME_SIZE)
  VOLUME_NAME := $(call qstrip,$(CONFIG_VOLUME_NAME))
  VOLUME_FLAGS := $(call qstrip,$(CONFIG_VOLUME_FLAGS))
  VOLUME_ALIGNMENT := $(CONFIG_VOLUME_ALIGNMENT)
endif
#<-----Sean@liteon, 2013-01-01
...
ifneq ($(CONFIG_TARGET_ROOTFS_INITRAMFS),)
  ...
  ifneq ($(CONFIG_TARGET_ROOTFS_UBIFS),)
    # Add for auto generate ubinize.cfg, Sean@liteon, 2013-04-23----->
    define Image/mkfs/ubifs/auto_cfg
            	@printf "[ubifs]\n" > $(KDIR)/ubinize.cfg
		@printf "mode=ubi\n" >> $(KDIR)/ubinize.cfg
		@printf "image=%s\n" $(UBIFS_IMAGE_NAME) >> $(KDIR)/ubinize.cfg
		@printf "vol_id=%s\n" $(VOLUME_ID) >> $(KDIR)/ubinize.cfg
		@printf "vol_size=%sMiB\n" $(VOLUME_SIZE) >> $(KDIR)/ubinize.cfg
		@[ "$(CONFIG_VOLUME_TYPE_dynamic)" = "y" ] && printf "vol_type=dynamic\n" >> $(KDIR)/ubinize.cfg  || printf "vol_type=static\n" >> $(KDIR)/ubinize.cfg
		@printf "vol_name=%s\n" $(VOLUME_NAME) >> $(KDIR)/ubinize.cfg
		@printf "vol_flags=%s\n" $(VOLUME_FLAGS) >> $(KDIR)/ubinize.cfg
		@printf "vol_alignment=%s\n" $(VOLUME_ALIGNMENT) >> $(KDIR)/ubinize.cfg
    endef
    # <-----Sean@liteon, 2013-04-23
    define Image/mkfs/ubifs
		@# Modified by Sean@liteon
		$(call Image/mkfs/ubifs/auto_cfg)	
