...
/* Add WDT macro definition, Sean@liteon, 2013-08-09 */
#ifdef LITEON_SRC_VALID
#define WDT_DEV	"/dev/wp688_wdt"
...
/* Add WDT setting function, Sean@liteon, 2013-08-09 */
#ifdef LITEON_SRC_VALID
void watchdog_usage(void)
{
    printf("Watchdog Usage: \n");
    printf("	gpio wdt enable  -   enable watchdog\n");
    printf("	gpio wdt disable -   disable watchdog\n");
    printf("	gpio wdt dontfeed   -   enable watch dog, but don't feed dog\n\n");
}

int gpio_set_watchdog(int argc, char **argv)
{
	int fd;
	int cmd = -1;
	
	fd = open(WDT_DEV, O_RDONLY);
	...
	if((strcasecmp(argv[2], "enable") == 0) || (strcasecmp(argv[2], "e") == 0) ) {
		cmd = ENABLE_WATCHDOG;
	} else if((strcasecmp(argv[2], "disable") == 0) || (strcasecmp(argv[2], "d") == 0) ) {
		 cmd = DISABLE_WATCHDOG;
	} else if((strcasecmp(argv[2], "dontfeed") == 0) || (strcasecmp(argv[2], "df") == 0) ) {
		 cmd =DONT_FEED_DOG;
	} else {
		watchdog_usage();
		return -1;
	...
	if(ioctl(fd, cmd, 0xFF) < 0) {
		printf("execution command failed!\n");
		close(fd);
		return -1;
	}
...
void usage(char *cmd)
{
	...
	/* Add usage, Sean@liteon, 2013-08-09 */
#ifdef LITEON_SRC_VALID
	watchdog_usage();
#endif
...
int main(int argc, char *argv[])
{
	...
	switch (argv[1][0]) {
	case 'w':
		/* Invoke WDT setting function, Sean@liteon, 2013-08-09 */
#ifdef LITEON_SRC_VALID
		if(strcasecmp(argv[1], "wdt") == 0) {
			gpio_set_watchdog(argc, argv);
		} else 
#endif
		/* Sean@liteon, 2013-08-09 */
			gpio_test_write();
